...
class Client_extension:
    def __init__(self, *args, **kwargs):
        use_default_handlers = kwargs.pop("use_default_handlers", True)
        handlers = {}
        if use_default_handlers:
            handlers.update(notification_default_handlers)
        handlers.update(kwargs.get("notif_handlers", {}))
        kwargs["notif_handlers"] = handlers

        self.data_handler = {"default": lambda sid, x: logger.debug(x.hex("."))}
    ...
    def session_update_dt_anchor_ranging_rounds(self, sid, round_list):
        ...
        if payload[1:1] == 0:
            rtv = []
            logger.debug(bytearray(rtv).hex("."))
        else:
            rtv = list(payload[2:])
            logger.debug(bytearray(rtv).hex("."))
        return status, rtv
...
def show_device_state(payload):
    ...
    logger.info("Device -> %s", DeviceState(state).name)
...
def show_session_state(payload):
    ...
    logger.info(
        ...
        SessionStateChangeReason(reason).name,
    )
...
notification_default_handlers = {
    ...
    (Gid.Session, OidSession.UpdateMulticastList): lambda x: logger.info(
        "%s", MulticastControleeList(x)
    ),
    (Gid.Ranging, OidRanging.DataCredit): lambda x: logger.info(
        "%s", SessionDataCredit(x)
    ),
    (Gid.Ranging, OidRanging.DataTransferStatus): lambda x: logger.info(
        "%s", SessionDataTransfertStatus(x)
    ),
    ("default", "default"): lambda gid, oid, x: logger.warning(
        "Unexpected notification: %s", NotImplementedData(gid, oid, x)
    ),
}
