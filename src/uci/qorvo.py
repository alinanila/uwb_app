...
notification_default_handlers.update(
    {
        (Gid.Ranging, OidRanging.Start): lambda x: logger.info("%s", RangingData(x)),
        (Gid.Test, OidTest.Loopback): lambda x: logger.info(
            "%s", LoopBackTestOutput(x)
        ),
        (Gid.Test, OidTest.PeriodicTx): lambda x: logger.info(
            "%s", PeriodicTxTestOutput(x)
        ),
        (Gid.Test, OidTest.SsTwr): lambda x: logger.info("%s", TwrTestOutput(x)),
        (Gid.Test, OidTest.Rx): lambda x: logger.info("%s", RxTestOutput(x)),
        (Gid.Test, OidTest.PerRx): lambda x: logger.info("%s", PerRxTestOutput(x)),
    }
)

notification_default_handlers.update(
    {
        (Gid.Qorvo, OidQorvo.TestDiag): lambda x: logger.info(
            "%s", RangingDiagData(x)
        ),
        (Gid.Qorvo, OidQorvo.SessionDataXferStatusNtf): lambda x: logger.info(
            "%s", SessionDataTransferStatus(x)
        ),
        (Gid.Qorvo, OidQorvo.TestDebug): lambda x: logger.info(
            "%s", TestDebugData(x)
        ),
        (Gid.Qorvo, OidQorvo.TestPllLock): lambda x: logger.info(
            "%s", PllLockTestOutput(x)
        ),
        (Gid.Qorvo, OidQorvo.TestRtc): lambda x: logger.info("%s", RtcTestOutput(x)),
        (Gid.Qorvo, OidQorvo.TestTof): lambda x: logger.info("%s", TofTestOutput(x)),
    }
)
