...
def parse_android_short_addr(s: str) -> int:
    """
    Parse Android-style 2-byte short address formatted as 'AB:01'.
    The Android app displays bytes as AB:01 but UCI expects 0x01AB.
    """
    a, b = s.strip().split(":")
    if len(a) != 2 or len(b) != 2:
        raise ValueError(f"Bad short address {s!r}; expected format 'AB:01'")
    return int(b + a, 16)
...
def main():
    ...
    parser.add_argument(
        ...
        "You can also specify the Android style short address e.g. AB:01",
    )
    parser.add_argument(
        "--dest-mac",
        type=str,
        help="set the DST_MAC_ADDRESS value which is a list.\n"
        "default: [0x0] if controlee or [0x1].\n"
        "You can also specify a single Android style short address e.g. AB:01",
    )
    parser.add_argument(
    ...
    try:
        ...
        # Accept Android-style short address formats for --mac / --dest-mac
        # e.g. "AB:01" -> 0x01AB
        if hasattr(args, "mac") and isinstance(getattr(args, "mac"), str):
            if ":" in getattr(args, "mac"):
                setattr(args, "mac", parse_android_short_addr(getattr(args, "mac")))

        if hasattr(args, "dest_mac") and isinstance(getattr(args, "dest_mac"), str):
            if ":" in getattr(args, "dest_mac"):
                setattr(
                    args,
                    "dest_mac",
                    [parse_android_short_addr(getattr(args, "dest_mac"))],
                )
        ...
        for p in (
            ...
            if hasattr(args, p):
                v = getattr(args, p)
                # Only eval strings here, because mac and dest_mac might already be converted to an int or list by Android-style format/preprocessing
                # So running replace() on those ints/lists would cause an error
                if isinstance(v, str):
                    setattr(args, p, eval(v.replace("-", "_"), user_mapping))
        for p in ["skey", "sskey"]:
            if hasattr(args, p):
            ...
            if "controlees_with_sskey" in vars(args):
                try:
                    ...
                    for i in range(len(controlees_with_sskey) // 3):
                        # Allow Android-style short address in controlees-with-sskey triples
                        # triple format: [short_addr, ssID, ssKey, ...]
                        short_addr = controlees_with_sskey[3 * i + 0]
                        if isinstance(short_addr, str) and ":" in short_addr:
                            controlees_with_sskey[3 * i + 0] = parse_android_short_addr(short_addr)
